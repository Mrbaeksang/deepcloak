...
> **实测验证 —— 非演示造假。** 一次真实的 `deepcloak --depth detailed` 运行抓取了 **32 个来源并绕过 11 道 Cloudflare/Turnstile 墙**（cloudflare.com、blog.cloudflare.com、medium.com、capmonster.cloud……），开放页面保持快速路径。上方演示即回放该会话 —— 自行验证：`asciinema play` [`docs/media/demo.cast`](docs/media/demo.cast)（原始录制）或观看 [`docs/media/demo-terminal.mp4`](docs/media/demo-terminal.mp4)（纯终端）。
